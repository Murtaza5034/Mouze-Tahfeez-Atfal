// Auto-generated from alquran.cloud quran-uthmani (standard Madani mushaf, 604-page layout).
// QURAN_PAGE_STARTS[page - 1] = [surahNumber, ayahNumber] of the first ayah printed on that page.

export const QURAN_PAGE_STARTS = [
  [1, 1],
  [2, 1],
  [2, 6],
  [2, 17],
...
export const JUZ_PAGE_MAP = [1,22,42,62,82,102,122,142,162,182,202,222,242,262,282,302,322,342,362,382,402,422,442,462,482,502,522,542,562,582];
...
// Exact page lookup: binary search for the last page whose start (surah, ayah) <= target.
export const getAyahPage = (surahNum, ayahNum) => {
  const s = Number(surahNum);
  const a = Number(String(ayahNum).split('-')[0]);
  if (!Number.isFinite(s) || !Number.isFinite(a) || s < 1 || a < 1) return 1;
  let lo = 0;
  let hi = QURAN_PAGE_STARTS.length - 1;
  let ans = 0;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    const [ms, ma] = QURAN_PAGE_STARTS[mid];
    if (ms < s || (ms === s && ma <= a)) {
      ans = mid;
      lo = mid + 1;
    } else {
