 ...
 * The open panel renders through a portal into document.body with fixed
 * positioning so it is never clipped or covered by card containers.
 * props:
...
export default function SearchableSelect({
  ...
  const positionPanel = () => {
    ...
    const panelH = panelRef.current ? panelRef.current.offsetHeight : 0;
    const spaceBelow = window.innerHeight - r.bottom - 8;
    const useFlip = panelH > 0 && spaceBelow < panelH && r.top > panelH;
    setFlip(useFlip);
    setCoords({
      top: useFlip ? Math.max(8, r.top - panelH - 8) : r.bottom + 8,
      left: r.left,
      width: r.width,
    });
  };
  ...
  const panel = open ? createPortal(
    <div
      ref={panelRef}
      className={`searchable-select-panel ${flip ? "is-flipped" : ""}`}
      role="listbox"
      style={{
        position: 'fixed',
        top: coords?.top ?? 0,
