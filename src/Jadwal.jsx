...
const handleDownloadPDF = async (studentName, scheduleData, mode = 'juz-wise', theme = {}, style = 'table', dayDates = [], onDownloadComplete = null) => {
  ...
  for (let pi = 0; pi < pageGroups.length; pi++) {
    ...
    if (pi > 0) pdf.addPage();
    let imgWidth2 = printableWidth;
    let imgHeight2 = (canvas.height * imgWidth2) / canvas.width;
    if (imgHeight2 > printableHeight) {
      imgWidth2 = (imgWidth2 * printableHeight) / imgHeight2;
      imgHeight2 = printableHeight;
    }
    const xOffset2 = (printableWidth - imgWidth2) / 2;
    pdf.addImage(imgData, "JPEG", xOffset2, 0, imgWidth2, imgHeight2, undefined, 'FAST');
